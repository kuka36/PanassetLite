...
import { Asset, AssetType, Currency, VoiceParseResult, TransactionType, AssetMetadata } from '../types/domain';
import { KNOWN_MANUAL_SYMBOLS } from '../services/marketData';
import { usePortfolio } from '../context/PortfolioContext';
import { VoiceInput } from './ui/VoiceInput';
import { TagManagementModal } from './TagManagementModal';
import { X, Save, TrendingUp, Bitcoin, PieChart, Building, Banknote, CreditCard, Box, Calendar, PenLine, Tag } from 'lucide-react';
...
export const AddAssetModal: React.FC<Props> = ({ isOpen, onClose, initialAsset }) => {
    const { addAsset, editAsset, addTransaction, t, assets } = usePortfolio();
    ...
    useEffect(() => {
        if (isOpen) {
            if (initialAsset) {
                ...
                setSelectedTags(initialAsset.tags || []);
                setStep(2);
            } else {
                // Add Mode - Reset
                setStep(1);
                ...
                setCost('');
                // 自定义资产默认当前价为 1
                setCurrentVal(isStrictManualAsset(AssetType.STOCK) ? '1' : '');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const assetMeta: AssetMetadata = {
            id: initialAsset ? initialAsset.id : SequenceService.generateId(),
                        ...
                        <form id="asset-form" onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-3">
                                <div className="flex justify-between items-center">
                                    ...
                                    >
                                        {selectedTags.length > 0 ? t('editTags') || '编辑标签' : t('addTags') || '添加标签'}
                                    </button>
                                </div>
